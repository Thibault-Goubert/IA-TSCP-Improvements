...
{
	memset(canAttack, 0, sizeof(canAttack)); // Pour test

	for (int i = 1; i < 6; i++) { // pour chaque piece de knight(1) à roi(5)
		for (int k = 0; k < 64; k++) { // pour chaque position sur le plateau
			for (int j = 0; j < offsets[i]; ++j) { //Pour chaque direction posibles pour la piece
				for (int n = k;;) { //Boucle infini
					n = mailbox[mailbox64[n] + offset[i][j]]; // n = la case d'arrivé en prenant en compte les boards
					if (n == -1) break; // Si n = -1 move interdit 
					canAttack[i][k][n] = 1;
					if (!slide[i]) break; // Si l'on ne peux pas slide, 
				}
			}
		}
	}
